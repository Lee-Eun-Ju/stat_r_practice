...
write.csv(newdata, "C:\\Users\\이은주\\Desktop\\AIR 동아리\\블랙프라이데이\\EDA\\newdata.csv")


newdata <- read.csv("C:\\Users\\이은주\\Desktop\\AIR 동아리\\블랙프라이데이\\EDA\\newdata.csv")
head(newdata)
str(newdata)
...
# Stay_In_Current_City_Years -> 범주형 자료는 sort하여 자료 확인
...
# Purchase
sort(table(newdata$Purchase),decreasing=T)[1:50] #내림차순
